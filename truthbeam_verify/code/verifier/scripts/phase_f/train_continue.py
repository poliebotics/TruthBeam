...
def main():
    ...
    ap.add_argument("--resume-from", type=Path, default=None,
                    help="Resume training from a full editor checkpoint.")
    ap.add_argument("--save-every", type=int, default=0,
                    help="If >0, save step_NNNNNN.pt every this many steps.")
    ap.add_argument("--start-step", type=int, default=0,
                    help="Starting step counter (for resumed runs).")
    args = ap.parse_args()
    ...
    if args.resume_from is not None:
        ...
        state = ck["model"] if "model" in ck else ck.get("editor", ck)
        miss, unexp = editor.load_state_dict(state, strict=False)
        print(f"[init] resume from {args.resume_from.name}: missing={len(miss)} unexp={len(unexp)} prev_step={ck.get('step', '?')}", flush=True)
    ...
    while step < end_step:
        ...
        if args.save_every > 0 and step > args.start_step and step % args.save_every == 0:
            ckpt_path = args.out_dir / f"step_{step:06d}.pt"
