...
def main() -> None:
    ...
    ap.add_argument("--resume-from", type=Path, default=None,
                    help="Path to a checkpoint to resume training from. The "
                    "checkpoint's 'step' is used as the starting step; model "
                    "and optimizer state are restored. history.jsonl is "
                    "appended to (not truncated) and the overfit canary is "
                    "skipped automatically since the model is already "
                    "trained. Pair with --max-steps to set the new target.")
    args = ap.parse_args()
    ...
    # If --resume-from given, load model + optimizer state and set start_step
    # so the cosine LR schedule + ckpt cadence pick up where the previous run
    # left off. The overfit canary is automatically skipped on resume (the
    # model is already trained; canary would not be informative).
    resume_step = 0
    if args.resume_from is not None:
        ...
        critical_keys = ("max_steps", "bs", "lr", "seed")
        mismatches = {}
        for k in critical_keys:
            if k in ck_args:
                old = ck_args[k]
                new = getattr(args, k, None)
                if old != new:
                    mismatches[k] = (old, new)
        if mismatches and is_main:
            print(f"[resume WARN] ckpt args differ from current invocation: "
                  f"{mismatches}. LR schedule and convergence may differ from "
                  "the original run. Set matching args to silence this warning.",
                  flush=True)
        # In DDP, broadcast loaded params from rank 0 to ensure all ranks
        # have identical state (avoids subtle drift if different ranks
        # got slightly different checkpoint loads).
        if world_size > 1:
            for p in (model.module if hasattr(model, "module") else model).parameters():
                dist.broadcast(p.data, src=0)
        # Force-skip overfit canary on resume (model is post-training).
        args.skip_overfit_canary = True
        if is_main:
            print(f"[resume] resumed at step={resume_step}, target max_steps={args.max_steps}",
                  flush=True)
    ...
    if is_main:
        ...
        # On fresh start (resume_step == 0), truncate. On resume, append.
        if resume_step == 0 and history_path.exists():
            history_path.unlink()
