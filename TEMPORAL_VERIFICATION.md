...
`code/recording/verify/CLAIMS.md`) to **demonstrated for these
...
readings; drand freshness is per-**round** (3 s beacon; measured folding one fresh round per ~11-13 frames, ~5-6 s), not per-frame; the window is no tighter
...
window for **D2** from four public pages from two independent systems (the Rootstock explorer and the
drand API) - no tools beyond a browser:
...
Those four links alone establish the **[02:07:48 → 02:48:47 UTC] window** across the two systems. (V10
is identical, with blocks **8769289 / 8769357**.) The scripted checks below add per-transaction calldata
confirmation and per-round BLS verification - but the window itself needs only a browser.
...
refreshed at a measured ~5-6 s (one fresh round per ~11-13 frames; the 3 s beacon is polled, and roughly every other round is skipped).
...
- **RSK pulses - all included, calldata-confirmed, continuous, monotonic.** D2 **161/161** and V10
  **102/102** anchor transactions (state pulses plus each session's final-root tx: 160+1 and 101+1) are confirmed on-chain, landing in monotonically-increasing mainnet
...
- **drand - every round BLS-verified.** All **476** (D2) and **324** (V10) folded quicknet rounds were
  refetched and pass the BLS pairing-check - **zero failures**. The rounds advance monotonically
  through the session (typically one to two beacon periods per step; max observed staleness 16.6 s)
  from the session-open round to the session-close round, bracketing the window.
- **Commit rate is rock-steady.** Inter-frame intervals: **median 0.400 s, p95 0.401 s** (max 0.501 s = an
  occasional single-frame gap) → a near-constant **2.50 Hz**, ≈ one BLS-verifiable fresh drand challenge per
  ~11-13 frames (measured: D2 12.6, V10 11.6).

**Scope.** This demonstrates the *time window* and *commit rate* for the two released sessions. The
freshness floor is established at **folded-round granularity** (measured ~5-6 s / ~11-13 frames; max staleness 16.6 s): every folded round is
